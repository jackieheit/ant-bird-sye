...
```{r}
pois_norsa <- glm(Successes ~ species + roa + isa + ioa + species*roa + species*isa + species*ioa, family = "poisson", data = bird_ind)
anova(pois_norsa, pois_full, test = "Chisq")

pois_noroa <- glm(Successes ~ species + rsa + isa + ioa + species*rsa + species*isa + species*ioa, family = "poisson", data = bird_ind)
anova(pois_noroa, pois_full, test = "Chisq")

pois_noisa <- glm(Successes ~ species + rsa + roa + ioa + species*rsa + species*roa + species*ioa, family = "poisson", data = bird_ind)
anova(pois_noisa, pois_full, test = "Chisq")

pois_noioa <- glm(Successes ~ species + rsa + roa + isa + species*rsa + species*roa + species*isa , family = "poisson", data = bird_ind)
anova(pois_noioa, pois_full, test = "Chisq")
```

```{r}
nb_norsa <- glm.nb(Successes ~ species + roa + isa + ioa + species*roa + species*isa + species*ioa, data = bird_ind)

nb_noroa <- glm.nb(Successes ~ species + rsa + isa + ioa + species*rsa + species*isa + species*ioa, data = bird_ind)

nb_noisa <- glm.nb(Successes ~ species + rsa + roa + ioa + species*rsa + species*roa + species*ioa, data = bird_ind)

nb_noioa <- glm.nb(Successes ~ species + rsa + roa + isa + species*rsa + species*roa + species*isa, data = bird_ind)


anova(nb_norsa, nbinom_full)
anova(nb_noroa, nbinom_full)
anova(nb_noisa, nbinom_full)
anova(nb_noioa, nbinom_full)
```
...
```{r}
step.model <- step(nbinom_full,direction="backward")
```
...
```{r}
zinb_model <- zeroinfl(Successes ~ species + rsa + roa + species*rsa + species*roa, data = bird_ind, dist = "negbin")
summary(zinb_model)
```
